...
namespace
{
  std::wstring Utf8ToWide(const std::string& value)
  {
    ...
    const int required_len = MultiByteToWideChar(CP_UTF8, 0, value.c_str(), -1, nullptr, 0);
    if (required_len <= 0) {
      return L"";
    }
    std::wstring wide(static_cast<size_t>(required_len), L'\0');
    MultiByteToWideChar(CP_UTF8, 0, value.c_str(), -1, wide.data(), required_len);
    wide.pop_back();
    return wide;
  }
  ...
  std::string WideToUtf8(const wchar_t* value)
  {
    ...
    const int required_len = WideCharToMultiByte(CP_UTF8, 0, value, -1, nullptr, 0, nullptr, nullptr);
    if (required_len <= 0) {
      return "";
    }
    std::string utf8(static_cast<size_t>(required_len), '\0');
    WideCharToMultiByte(CP_UTF8, 0, value, -1, utf8.data(), required_len, nullptr, nullptr);
    utf8.pop_back();
    return utf8;
  }
