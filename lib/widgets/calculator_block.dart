...
class _CalculatorBlockWidgetState extends State<CalculatorBlockWidget> {
  ...
  Widget _chip(String t, bool on, VoidCallback f) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: on
            ? const Color(0xFFFFAA5C).withAlpha(30)
      ...
      child: Text(
        t,
        style: TextStyle(
          color: on ? const Color(0xFFFFAA5C) : Colors.white70,
          fontSize: 11,
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: Text(
                  'Result: ${_result.isEmpty ? '-' : _result}',
                  style: TextStyle(
                    color: _result == 'Error'
          ...
          (i) => Padding(
            ...
            child: TextField(
              controller: TextEditingController(text: _y[i])
                ..selection = TextSelection.collapsed(offset: _y[i].length),
              onChanged: (v) {
                _y[i] = v;
                _save();
              },
...
class _GraphPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    ...
    final g = Paint()
      ...
      ..strokeWidth = 1;
    for (var i = 0; i <= 10; i++) {
      final x = s.width * i / 10, y = s.height * i / 10;
...
class _Parser {
  ...
  double _fn(String n, double a) {
    final k = n.toLowerCase();
    final t = (deg && (k == 'sin' || k == 'cos' || k == 'tan'))
        ? a * math.pi / 180
        : a;
    switch (k) {
      case 'sin':
